...
export function JobSearch({
  ...
  function runSearch() {
    ...
    startTransition(async () => {
      const params = new URLSearchParams({ needType: "JOB" });
      ...
      setResults((data.requests ?? []) as JobHit[]);
    });
  }
    ...
    <section className="space-y-4">
      {!hideHeading && (
        <div>
          ...
          <p className="text-sm text-[var(--muted)]">{t("search_jobs_hint")}</p>
        </div>
      )}
          ...
          <div>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <div className="space-y-1.5 sm:col-span-2 lg:col-span-1">
                <Label htmlFor="job-q">{t("search")}</Label>
                <Input
                  id="job-q"
              ...
              <div className="space-y-1.5">
                <Label htmlFor="job-region">{t("region")}</Label>
                <Select
                  id="job-region"
                  value={region}
                  onChange={(e) => {
              ...
              <div className="space-y-1.5">
                <Label htmlFor="job-country">{t("country")}</Label>
                <Select
                  id="job-country"
                  value={country}
                  onChange={(e) => {
      ...
      {searched && (
        <div className="space-y-3">
          <p className="text-sm text-[var(--muted)]">
            {results.length} {t("jobs_found")}
          </p>
          {results.map((hit) => {
            const cover = hit.photos[0];
            const title =
              hit.jobTitle?.trim() ||
              (hit.needType === "JOB_OFFER"
                ? t("order_need_job_offer")
                : t("order_need_job_seek"));
            const place =
              [hit.toCity, hit.toCountry ? getCountryName(hit.toCountry) : ""]
                .filter(Boolean)
                .join(", ") || "—";
            return (
                  ...
                  <div className="flex min-w-0 flex-1 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        ...
                        {hit.jobSector && (
                          <Badge>
                            {jobSectorLabel(hit.jobSector, locale)}
                          </Badge>
                        )}
                        {hit.jobExperience && (
                          <Badge>
                            {jobExperienceLabel(hit.jobExperience, locale)}
                          </Badge>
                        )}
          ...
          {results.length === 0 && (
            <p className="text-sm text-[var(--muted)]">{t("no_jobs_found")}</p>
          )}
        </div>
      )}
    </section>
